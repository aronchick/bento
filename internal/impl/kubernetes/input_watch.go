...
func (k *kubernetesWatchInput) watchNamespace(namespace string) {
	...
	for {
		...
		if k.includeInitialList && k.resourceVers == "" {
			k.listExistingResources(resourceInterface)
		}

		// Use stored resource version if available
		if k.resourceVers != "" {
			listOpts.ResourceVersion = k.resourceVers
		}
		...
		if err != nil {
			...
			// Reset resource version on error - it may be too old
			k.resourceVers = ""
			continue
		}
...
func (k *kubernetesWatchInput) listExistingResources(resourceInterface dynamic.ResourceInterface) {
	listOpts := metav1.ListOptions{
		LabelSelector: k.labelSelector,
		FieldSelector: k.fieldSelector,
	}

	list, err := resourceInterface.List(context.Background(), listOpts)
	if err != nil {
	...
	k.mu.Lock()
	k.resourceVers = list.GetResourceVersion()
	k.mu.Unlock()
	...
	for {
		...
		case event, ok := <-watcher.ResultChan():
			...
			case watch.Added, watch.Modified, watch.Deleted:
				...
				k.mu.Lock()
				k.resourceVers = obj.GetResourceVersion()
				k.mu.Unlock()
